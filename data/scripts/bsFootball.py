...
class FootballCoopGame(bs.CoopGameActivity):
    ...
    def __init__(self,settings={}):
        ...
        # self._timeElapsedText = bs.Lstr(resource='timeElapsedText').evaluate()
        ...
        self._scoreRegionMaterial.addActions(conditions=("theyHaveMaterial",bs.Flag.getFactory().flagMaterial),
                                             actions=(("modifyPartCollision","collide",True),
                                                      ("modifyPartCollision","physical",False),
                                                      ("call","atConnect",self._handleScore)))
    ...
    def onBegin(self):
        ...
        if self._preset in ['rookie','rookieEasy']:
            ...
            self._botTypes14 = [cb]*(1 if len(self.initialPlayerInfo) < 3 else 2)
        elif self._preset == 'tournament':
            self._excludePowerups = []
            self._haveTnt = True
            self._botTypesInitial = [bs.ToughGuyBot]*(1 if len(self.initialPlayerInfo) < 2 else 2)
            self._botTypes7 = [bs.ChickBot]*(1 if len(self.initialPlayerInfo) < 3 else 2)
            self._botTypes14 = [bs.NinjaBot]*(1 if len(self.initialPlayerInfo) < 4 else 2)
            ...
            self._botTypes7 = [tgb]+[bs.BomberBot]*(1 if len(self.initialPlayerInfo) < 3 else 2)
            ...
            self._botTypesInitial = [bs.MelBot] + [tgb]*len(self.initialPlayerInfo)
            self._botTypes7 = [cb]*(1 if len(self.initialPlayerInfo) < 3 else 2)
            self._botTypes14 = [bs.PirateBot]*(1 if len(self.initialPlayerInfo) < 3 else 2)
    ...
    def _updateBots(self):
        ...
        if self._flag.node.exists():
            for p in self.players:
                try:
                    if p.actor.isAlive() and p.actor.node.holdNode == self._flag.node:
                        return
                except Exception:
            ...
            closestBot = None
            for bot in bots:
                if bot.heldCount > 0: continue # if a bot is picked up he should forget about the flag
    ...
    def _dropPowerups(self,standardPoints=False,powerupType=None):
        ...
        if standardPoints:
            ...
            for i,pt in enumerate(pts):
                bs.gameTimer(1000+i*500,bs.Call(self._dropPowerup,i,powerupType))
        else:
            ...
            if team.gameData['score'] >= winScore:
                if not haveScoringTeam:
                    ...
                    if team is self._botTeam:
                        ...
                        if self._preset in ['rookie','rookieEasy']:
                            self._awardAchievement('Rookie Football Victory',sound=False)
                            if self._botTeam.gameData['score'] == 0:
                                self._awardAchievement('Rookie Football Shutout',sound=False)
                        elif self._preset in ['pro','proEasy']:
                            self._awardAchievement('Pro Football Victory',sound=False)
                            if self._botTeam.gameData['score'] == 0:
                                self._awardAchievement('Pro Football Shutout',sound=False)
                        elif self._preset in ['uber','uberEasy']:
                            self._awardAchievement('Uber Football Victory',sound=False)
                            if self._botTeam.gameData['score'] == 0:
                                self._awardAchievement('Uber Football Shutout',sound=False)
                            if not self._playerHasDroppedBomb and not self._playerHasPunched:
                                self._awardAchievement('Got the Moves',sound=False)
    ...
    def doEnd(self,outcome):
        ...
        self.end(delay=3000,results={'outcome':outcome,'score':None if outcome == 'defeat' else int(self._finalTime/10),
                                     'scoreOrder':'decreasing','playerInfo':self.initialPlayerInfo})
    ...
    def spawnPlayer(self,player):
        s = self.spawnPlayerSpaz(player,position=self.getMap().getStartPosition(player.getTeam().getID()))
