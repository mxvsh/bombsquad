...
class Chooser(object):
    ...
    def _setReady(self,ready):
        ...
        if not ready:
            self._player.assignInputCall('leftPress',bs.Call(self.handleMessage,ChangeMessage('team',-1)))
            self._player.assignInputCall('rightPress',bs.Call(self.handleMessage,ChangeMessage('team',1)))
            self._player.assignInputCall('bombPress',bs.Call(self.handleMessage,ChangeMessage('character',1)))
            self._player.assignInputCall('upPress',bs.Call(self.handleMessage,ChangeMessage('profileIndex',-1)))
            self._player.assignInputCall('downPress',bs.Call(self.handleMessage,ChangeMessage('profileIndex',1)))
            self._player.assignInputCall(('jumpPress','pickUpPress','punchPress'), bs.Call(self.handleMessage,ChangeMessage('ready',1)))
            self.ready = False
            self._updateText()
...
class Lobby(object):
    ...
    def __del__(self):
        # reset any players that still have a chooser in us (should allow the choosers to die)
        players = [chooser._player for chooser in self.choosers if chooser._player.exists()]
    ...
    class JoinInfo(object):
        ...
        def __init__(self,lobby):
            ...
            # FIXME - this of course is only correct locally; will need to change this for net games
            kb = bsInternal._getInputDevice('Keyboard','#1',exceptionOnNone=False)
            if kb is not None:
                punchKey = kb.getButtonName(bsUI.getControllerValue(kb,'buttonPunch'))
                pressToPunch = bs.Lstr(resource='orText',subs=[('${A}',bs.Lstr(value='\'${K}\'',subs=[('${K}',punchKey)])),('${B}',pressToPunch)])
                bombKey = kb.getButtonName(bsUI.getControllerValue(kb,'buttonBomb'))
                pressToBomb = bs.Lstr(resource='orText',subs=[('${A}',bs.Lstr(value='\'${K}\'',subs=[('${K}',bombKey)])),('${B}',pressToBomb)])
                joinStr = bs.Lstr(value='${A} < ${B} >',subs=[('${A}',bs.Lstr(resource='pressPunchToJoinText')),('${B}',pressToPunch)])
            else:
            ...
            if bsUtils.gRunningKioskModeGame:
                ...
                self._messages = (([bs.Lstr(resource='pressToSelectTeamText',subs=[('${BUTTONS}',bs.getSpecialChar('leftArrow')+' '+bs.getSpecialChar('rightArrow'))])] if canSwitchTeams else [])
                                  + [bs.Lstr(resource='pressToSelectProfileText',subs=[('${BUTTONS}',bs.getSpecialChar('upArrow')+' '+bs.getSpecialChar('downArrow'))])]
                                  + [bs.Lstr(value='${A} < ${B} >',subs=[('${A}',bs.Lstr(resource='pressToOverrideCharacterText',subs=[('${BUTTONS}',bs.Lstr(resource='bombBoldText'))])),('${B}',pressToBomb)])]
                                  + [joinStr])
    ...
    def addChooser(self,player):
        self.choosers.append(Chooser(vPos=self._vPos,player=player,lobby=self))
    ...
    def removeAllChoosersAndKickPlayers(self):
        'removes all player choosers and kicks attached players out of the session'
        for chooser in list(self.choosers): # need to copy this; can change under us
            try:
    ...
    def removeChooser(self, player):
        ...
        if not found:
            ...
            print 'ERROR: removeChooser: chooser still present after removal',player
            import traceback
            traceback.print_stack()
