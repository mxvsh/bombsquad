...
class RunaroundGame(bs.CoopGameActivity):
    ...
    tips = ['Jump just as you\'re throwing to get bombs up to the highest levels.',
    ...
    def __init__(self, settings={}):
        ...
        except Exception: self._preset = 'pro'
        ...
        self._powerupSpread = (self._mapType.defs.boxes['powerupRegion'][6]*0.5,self._mapType.defs.boxes['powerupRegion'][8]*0.5)
    ...
    def onTransitionIn(self):
        ...
        self._score = bs.SecureInt(0) # we use this in place of a regular int to make it harder to hack scores
        self._timeBonus = 0

        self._scoreRegion = bs.NodeActor(bs.newNode('region',
                                                    attrs={'position':self.getMap().defs.boxes['scoreRegion'][0:3],
                                                           'scale':self.getMap().defs.boxes['scoreRegion'][6:9],
                                                           'type': 'box',
    ...
    def onBegin(self):
        ...
        if self._preset in ['pro','proEasy','tournament']:
            ...
            self._waves = [
                {'entries':[
                        ...
                        {'type':bs.BomberBot,'path':2} if playerCount > 1 else None,
                        {'type':bs.BomberBot,'path':1} if hard else None,
                        {'type':bs.BomberBot,'path':1} if playerCount > 2 else None,
                        {'type':bs.BomberBot,'path':1} if playerCount > 3 else None,
                        ]},
                {'entries':[
                        {'type':bs.BomberBot,'path':1} if hard else None,
                        {'type':bs.BomberBot,'path':2} if hard else None,
                        {'type':bs.BomberBot,'path':2},
                        {'type':bs.BomberBot,'path':2},
                        {'type':bs.BomberBot,'path':2} if playerCount > 3 else None,
                        {'type':bs.ToughGuyBot,'path':3},
                        {'type':bs.ToughGuyBot,'path':3},
                        {'type':bs.ToughGuyBot,'path':3} if hard else None,
                        {'type':bs.ToughGuyBot,'path':3} if playerCount > 1 else None,
                        {'type':bs.ToughGuyBot,'path':3} if playerCount > 2 else None,
                        ]},
                {'entries':[
                        {'type':bs.NinjaBot,'path':2} if hard else None,
                        {'type':bs.NinjaBot,'path':2} if playerCount > 2 else None,
                        {'type':bs.ChickBot,'path':2},
                        {'type':bs.ChickBot,'path':2} if playerCount > 1 else None,
                        ...
                        {'type':bs.BomberBot,'path':3},
                        {'type':bs.BomberBot,'path':3} if playerCount > 3 else None,
                        ]},
                {'entries':[
                        ...
                        {'type':bs.ChickBot,'path':3},
                        {'type':'spacing','duration':1000},
                        {'type':bs.ChickBot,'path':1} if (playerCount > 1 and hard) else None,
                        {'type':'spacing','duration':1000},
                        {'type':bs.ChickBot,'path':2} if playerCount > 2 else None,
                        {'type':'spacing','duration':1000},
                        {'type':bs.ChickBot,'path':3} if playerCount > 3 else None,
                        {'type':'spacing','duration':1000},
                        ]},
                {'entries':[
                        {'type':bs.NinjaBotProShielded if hard else bs.NinjaBot,'path':1},
                        ...
                        {'type':bs.ToughGuyBot,'path':3} if playerCount > 1 else None,
                        {'type':bs.ToughGuyBot,'path':3} if playerCount > 2 else None,
                        {'type':bs.ToughGuyBot,'path':3} if playerCount > 3 else None,
                        ]},
                {'entries':[
                        ...
                        {'type':'spacing','duration':1500},
                        {'type':bs.BomberBotProShielded,'path':1} if hard else None,
                        {'type':'spacing','duration':1000} if hard else None,
                        {'type':bs.BomberBotProShielded,'path':3},
                        {'type':'spacing','duration':1500},
                        {'type':bs.BomberBotProShielded,'path':2},
                        ...
                        {'type':bs.BomberBotProShielded,'path':3} if playerCount > 1 else None,
                        {'type':'spacing','duration':1500},
                        {'type':bs.BomberBotProShielded,'path':2} if playerCount > 2 else None,
                        {'type':'spacing','duration':1500},
                        {'type':bs.BomberBotProShielded,'path':1} if playerCount > 3 else None,
                        ]},
                ]
            ...
            self._waves = [
                {'entries':[
                        ...
                        {'type':bs.ChickBot,'path':2},
                        {'type':bs.ChickBot,'path':3},
                        {'type':bs.ToughGuyBotPro if hard else bs.ToughGuyBot,'point':'BottomLeft'},
                        {'type':bs.ToughGuyBotPro,'point':'BottomRight'} if playerCount > 2 else None,
                        ]},
                {'entries':[
                        ...
                        {'type':bs.NinjaBot,'path':2},
                        {'type':bs.NinjaBot,'path':3},
                        {'type':bs.NinjaBot,'path':1} if playerCount > 2 else None,
                        ]},
                {'entries':[
                        {'type':bs.BomberBotProShielded,'path':1} if hard else None,
                        {'type':bs.BomberBotProShielded,'path':2},
                ...
                {'entries':[
                        ...
                        {'type':bs.ChickBotPro,'path':1 if hard else 2},
                        {'type':bs.ChickBotPro,'path':1 if hard else 2},
                        {'type':bs.ChickBotPro,'path':1 if hard else 2} if playerCount > 1 else None,
                        {'type':bs.ChickBotPro,'path':1 if hard else 2} if playerCount > 3 else None,
                        ]},
                {'entries':[
                        {'type':bs.ChickBotProShielded if hard else bs.ChickBotPro,'point':'BottomLeft'},
                        {'type':bs.ChickBotProShielded,'point':'BottomRight'} if hard else None,
                        {'type':bs.ChickBotProShielded,'point':'BottomRight'} if playerCount > 2 else None,
                        {'type':bs.BomberBot,'path':3},
                        {'type':bs.BomberBot,'path':3},
                ...
                {'entries':[
                        {'type':bs.BomberBotProShielded,'path':2},
                        {'type':bs.BomberBotProShielded,'path':2} if hard else None,
                        {'type':bs.MelBot,'point':'BottomRight'},
                        {'type':bs.BomberBotProShielded,'path':2},
                        {'type':bs.BomberBotProShielded,'path':2},
                        {'type':bs.MelBot,'point':'BottomRight'} if playerCount > 2 else None,
                        {'type':bs.BomberBotProShielded,'path':2},
                        {'type':bs.PirateBot,'point':'BottomLeft'},
                        {'type':bs.BomberBotProShielded,'path':2},
                        {'type':bs.BomberBotProShielded,'path':2} if playerCount > 1 else None,
                        {'type':'spacing','duration':5000},
        ...
        self._livesText = bs.NodeActor(bs.newNode('text',
                                                  attrs={'vAttach':'top',
                                                         'hAttach':'right',
                                                         'hAlign':'center',
    ...
    def _handleReachedEnd(self):
        ...
        if not spaz.isAlive(): return # ignore bodies flying in..
        ...
        if self._lives > 0:
            ...
            for i in range(4):
                bs.gameTimer(t,bs.Call(_safeSetAttr,self._livesText.node,'color',(1,0,0,1.0)))
                bs.gameTimer(t,bs.Call(_safeSetAttr,self._livesBG.node,'opacity',0.5))
                t += 125
                bs.gameTimer(t,bs.Call(_safeSetAttr,self._livesText.node,'color',(1.0,1.0,0.0,1.0)))
                bs.gameTimer(t,bs.Call(_safeSetAttr,self._livesBG.node,'opacity',1.0))
                t += 125
            bs.gameTimer(t,bs.Call(_safeSetAttr,self._livesText.node,'color',(0.8,0.8,0.8,1.0)))
    ...
    def _dropPowerups(self,standardPoints=False,forceFirst=None):
        ...
        # if its been a minute since our last wave finished emerging, stop giving out land-mine powerups.
        # (prevents players from waiting around for them on purpose and filling the map up)
        if bs.getGameTime() - self._lastWaveEndTime > 60000: extraExcludes = ['landMines']
        else: extraExcludes = []

        if standardPoints:
            pts = self.getMap().powerupSpawnPoints
            for i,pt in enumerate(pts):
                bs.gameTimer(1000+i*500,bs.Call(self._dropPowerup,i,forceFirst if i == 0 else None))
        else:
            ...
            bs.Powerup(position=pt,powerupType=bs.Powerup.getFactory().getRandomPowerupType(excludeTypes=self._excludePowerups+extraExcludes)).autoRetain()

    def endGame(self):
        # FIXME FIXME FIXME - if we don't start our bots moving again we get stuck
        # this is because the bot-set never prunes itself while movement is off and
        # onFinalize never gets called for some bots because _pruneDeadObjects() saw them as dead
        # and pulled them off the weak-ref lists.
        # this is an architectural issue; can hopefully fix this by having _actorWeakRefs not look at exists()
        self._bots.startMoving()
    ...
    def doEnd(self,outcome):
        ...
        self.end(delay=delay,results={'outcome':outcome,'score':score,'failMessage':failMessage,'playerInfo':self.initialPlayerInfo})
    ...
    def _updateWaves(self):
        ...
        if self._canEndWave and not self._bots.haveLivingBots() and not self._gameOver and self._lives > 0:
            ...
            if self._preset in ['endless','endlessTournament']: won = False
            else: won = (self._wave == len(self._waves))

            # reward time bonus
            baseDelay = 4000 if won else 0
            if self._timeBonus > 0:
                bs.gameTimer(0,bs.Call(bs.playSound,self._cashRegisterSound))
                bs.gameTimer(baseDelay,bs.Call(self._awardTimeBonus,self._timeBonus))
                baseDelay += 1000
            ...
            if won:
                ...
                if self._preset in ['pro','proEasy']:
                    ...
                    if self._lives == self._startLives: self._awardAchievement('The Wall',sound=False)
                    if not self._playerHasPickedUpPowerup: self._awardAchievement('Precision Bombing',sound=False)
                        
                elif self._preset in ['uber','uberEasy']:
                    self._awardAchievement('Uber Runaround Victory',sound=False)
                    if self._lives == self._startLives: self._awardAchievement('The Great Wall',sound=False)
                    if not self._aPlayerHasBeenKilled: self._awardAchievement('Stayin\' Alive',sound=False)
            ...
            if self._wave > 1: self.celebrate(500)
    ...
    def _awardCompletionBonus(self):
        ...
        bs.PopupText(bs.Lstr(value='+${A} ${B}',subs=[('${A}',str(bonus)),('${B}',bs.Lstr(resource='completionBonusText'))]),
                     color=(0.7,0.7,1.0,1),
                     scale=1.6,
    ...
    def _awardLivesBonus(self,):
        ...
        bs.playSound(self._cashRegisterSound)
        bs.PopupText(bs.Lstr(value='+${A} ${B}',subs=[('${A}',str(bonus)),('${B}',bs.Lstr(resource='livesBonusText'))]),
                     color=(0.7,1.0,0.3,1),
                     scale=1.3,
    ...
    def _awardFlawlessBonus(self):
        bs.playSound(self._cashRegisterSound)
        bs.PopupText(bs.Lstr(value='+${A} ${B}',subs=[('${A}',str(self._flawlessBonus)),('${B}',bs.Lstr(resource='perfectWaveText'))]),
                     color=(1,1,0.2,1),
                     scale=1.2,
    ...
    def _startTimeBonusTimer(self):
        self._timeBonusTimer = bs.Timer(1000,self._updateTimeBonus,repeat=True)
    ...
    def _startNextWave(self):
        ...
        if self._preset in ['endless','endlessTournament']:
            ...
            spazTypes = []
            if level < 6: spazTypes += [[bs.BomberBot,5.0]]
            if level < 10: spazTypes += [[bs.ToughGuyBot,5.0]]
            if level < 15: spazTypes += [[bs.ChickBot,6.0]]
            if level > 5: spazTypes += [[bs.ChickBotPro,7.5]]*(1+(level-5)/7)
            if level > 2: spazTypes += [[bs.BomberBotProShielded,8.0]]*(1+(level-2)/6)
            if level > 6: spazTypes += [[bs.ChickBotProShielded,12.0]]*(1+(level-6)/5)
            if level > 1: spazTypes += [[bs.NinjaBot,10.0]]*(1+(level-1)/4)
            if level > 7: spazTypes += [[bs.NinjaBotProShielded,15.0]]*(1+(level-7)/3)

            # bot type, their effect on target points
            defenderTypes = [[bs.BomberBot,0.9],
                             [bs.ToughGuyBot,0.9],
                             [bs.ChickBot,0.85]]
            if level > 2: defenderTypes += [[bs.NinjaBot,0.75]]
            if level > 4: defenderTypes += [[bs.MelBot,0.7]]*(1+(level-5)/6)
            if level > 6: defenderTypes += [[bs.PirateBot,0.7]]*(1+(level-5)/5)
            if level > 8: defenderTypes += [[bs.ToughGuyBotProShielded,0.65]]*(1+(level-5)/4)
            if level > 10: defenderTypes += [[bs.ChickBotProShielded,0.6]]*(1+(level-6)/3)
            ...
            for group in range(groupCount):
                ...
                if path == 3: pass
                # harder path
                elif path == 2: thisTargetPoints *= 0.8
                # even harder path
                elif path == 1: thisTargetPoints *= 0.7
                # looping forward
                elif path == 4: thisTargetPoints *= 0.7
                # looping backward
                elif path == 5: thisTargetPoints *= 0.7
                # random
                elif path == 6: thisTargetPoints *= 0.7

                def _addDefender(defenderType,point):
                    #entries.append()
                    return thisTargetPoints*defenderType[1],{'type':defenderType[0],'point':point}

                # add defenders
                defenderType1 = defenderTypes[random.randrange(len(defenderTypes))]
                defenderType2 = defenderTypes[random.randrange(len(defenderTypes))]
                defender1 = defender2 = None
                if (group == 0) or (group == 1 and level > 3) or (group == 2 and level > 5):
                    if random.random() < min(0.75,(level-1)*0.11):
                        thisTargetPoints,defender1 = _addDefender(defenderType1,'BottomLeft')
                    if random.random() < min(0.75,(level-1)*0.04):
                        thisTargetPoints,defender2 = _addDefender(defenderType2,'BottomRight')
                ...
                for i,member in enumerate(range(memberCount)):
                    if path == 4: thisPath = i%3 # looping forward
                    elif path == 5: thisPath = 3-(i%3) # looping backward
                    elif path == 6: thisPath = random.randint(1,3) # random
                    else: thisPath = path
                    entries.append({'type':spazType[0],'path':thisPath})
                    if spacing != 0:
                        entries.append({'type':'spacing','duration':spacing})

                if defender1 is not None: entries.append(defender1)
                if defender2 is not None: entries.append(defender2)

                # some spacing between groups
                r = random.random()
                if r < 0.1: spacing = 5000
                elif r < 0.5: spacing = 1000
                else: spacing = 1
                entries.append({'type':'spacing','duration':spacing})
        ...
        for info in botTypes:
            ...
            if botType is not None:
                ...
                if botType == 'spacing':
                    ...
                    try: path = info['path']
                    except Exception: path = random.randint(1,3)
            ...
            t += int(delay*0.5)
            bs.gameTimer(t,bs.Call(self.addBotAtPoint,point,{'type':botType,'path':path},100 if point == 'Start' else nonRunnerSpawnTime))
            t += int(delay*0.5)

        # we can end the wave after all the spawning happens
        bs.gameTimer(t-int(delay*0.5)+nonRunnerSpawnTime+10,self._setCanEndWave)

        # reset our time bonus
        # in this game we use a constant time bonus so it erodes away in roughly the same time
        # (since the time limit a wave can take is relatively constant)
        # ..we then post-multiply a modifier to adjust points
        self._timeBonus = 150
        ...
        self._timeBonusText = bs.NodeActor(bs.newNode('text',
                                                      attrs={'vAttach':'top',
                                                             ...
                                                             'position':(0,-60),
                                                             'scale':0.8,
                                                             'text':bs.Lstr(value='${A}: ${B}',subs=[('${A}',bs.Lstr(resource='timeBonusText')),('${B}',str(int(self._timeBonus*self._timeBonusMult)))])}))
        ...
        self._waveText = bs.NodeActor(bs.newNode('text',
                                                 attrs={'vAttach':'top',
                                                        ...
                                                        'position':(0,-40),
                                                        'scale':1.3,
                                                        'text':bs.Lstr(value='${A} ${B}',subs=[('${A}',bs.Lstr(resource='waveText')),('${B}',str(self._wave)+('' if self._preset in ['endless','endlessTournament'] else ('/'+str(len(self._waves)))))])}))
    ...
    def addBotAtPoint(self,point,spazInfo,spawnTime=100):
        ...
        self._bots.spawnBot(spazInfo['type'],pos=pt,spawnTime=spawnTime,onSpawnCall=bs.Call(self._onBotSpawn,spazInfo['path']))
    ...
    def _updateTimeBonus(self):
        ...
        if self._timeBonus > 0 and self._timeBonusText is not None:
            self._timeBonusText.node.text = bs.Lstr(value='${A}: ${B}',subs=[('${A}',bs.Lstr(resource='timeBonusText')),('${B}',str(int(self._timeBonus*self._timeBonusMult)))])
    ...
    def _updateScores(self):
        ...
        if self._preset == 'endless':
            if score >= 500: self._awardAchievement('Runaround Master')
            if score >= 1000: self._awardAchievement('Runaround Wizard')
            if score >= 2000: self._awardAchievement('Runaround God')
    ...
    def _updateBot(self,bot):
        ...
        if bs.isPointInBox(t,boxes['b7']):
            ...
            bot.node.run = 0.0
            return True
        elif (bs.isPointInBox(t,boxes['b8']) and not bs.isPointInBox(t,boxes['b9'])) or t == (0.0,0.0,0.0):
    ...
    def _getBotSpeed(self,botType):
        if botType == bs.BomberBot: return 0.48
        elif botType == bs.BomberBotPro: return 0.48
        elif botType == bs.BomberBotProShielded: return 0.48
        elif botType == bs.ToughGuyBot: return 0.57
        elif botType == bs.ToughGuyBotPro: return 0.57
        elif botType == bs.ToughGuyBotProShielded: return 0.57
        elif botType == bs.ChickBot: return 0.73
        elif botType == bs.ChickBotPro: return 0.78
        elif botType == bs.ChickBotProShielded: return 0.78
        elif botType == bs.NinjaBot: return 1.0
        elif botType == bs.NinjaBotProShielded: return 1.0
        elif botType == bs.PirateBot: return 1.0
        elif botType == bs.MelBot: return 0.5
        else: raise Exception('Invalid bot type to _getBotSpeed(): '+str(botType))
