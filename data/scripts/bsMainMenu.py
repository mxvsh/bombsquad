        ...
        if l != self._language:
            ...
            if bs.getLanguage() == 'Chinese':
                ...
                for shadow in (True, False):
                    ...
                    self._makeWord('b',x,y+yExtra-10,delay=delay,scale=1.1*gScale,vrDepthOffset=5,shadow=shadow)
                    x += spacing*0.85; delay += delayInc

                    self._makeWord('S',x,y-25+0.8*yExtra,scale=1.35*gScale,delay=delay,vrDepthOffset=14,shadow=shadow)
                    x += spacing; delay += delayInc
                    ...
                    x += spacing*0.64; delay += delayInc
                    self._makeWord('d',x,y+yExtra-10,delay=delay,scale=1.1*gScale,vrDepthOffset=6,shadow=shadow)
