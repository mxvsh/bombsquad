...
class AssaultGame(bs.TeamGameActivity):
    ...
    def getSettings(cls,sessionType):
        return [("Score to Win",{'minValue':1,'default':3}),
                ("Time Limit",{'choices':[('None',0),('1 Minute',60),('2 Minutes',120),
                                          ('5 Minutes',300),('10 Minutes',600),('20 Minutes',1200)],
                               'default':0}),
                ("Respawn Times",{'choices':[('Shorter',0.25),('Short',0.5),('Normal',1.0),('Long',2.0),('Longer',4.0)],
                                  'default':1.0}),
    ...
    def getInstanceDescription(self):
        if self.settings['Score to Win'] == 1: return 'Touch the enemy flag.'
        else: return ('Touch the enemy flag ${ARG1} times.',self.settings['Score to Win'])
        
    def getInstanceScoreBoardDescription(self):
        if self.settings['Score to Win'] == 1: return 'touch 1 flag'
        else: return ('touch ${ARG1} flags',self.settings['Score to Win'])

    def onTransitionIn(self):
        bs.TeamGameActivity.onTransitionIn(self, music='Epic' if self.settings['Epic Mode'] else 'ForwardMarch')
        ...
        for team in self.teams:
            ...
            m.addActions(conditions=('theyHaveMaterial',bs.getSharedObject('playerMaterial')),
                         actions=(('modifyPartCollision','collide',True),
                                  ...
                                  ('call','atConnect',bs.Call(self._handleBaseCollide,team))))
    ...
    def _handleBaseCollide(self,team):
        ...
        if player is None or not player.exists() or not player.isAlive(): return
    ...
    def endGame(self):
        ...
        for team in self.teams: results.setTeamScore(team, team.gameData['score'])
        self.end(results=results)
        
    def _updateScoreBoard(self):
        for team in self.teams:
            self._scoreBoard.setTeamValue(team,team.gameData['score'],self.settings['Score to Win'])
