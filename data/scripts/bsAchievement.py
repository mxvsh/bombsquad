...
# simply display a completion banner for an achievement; used for server-driven achievements
...
def getAchievement(name):
    try: return [a for a in gAchievements if a._name == name][0]
    except Exception: raise Exception("Invalid achievement name: '"+name+"'")

# return the mult for achievement pts (just for display; changing this here won't affect what you get :-P)
...
def getAchievementsForCoopLevel(level):
    # for the Easy campaign we return achievements for the Default campaign too..
    # (want the user to see what achieements are part of the level even if they can't unlock them all on easy mode)
    return [a for a in gAchievements if a._level in (level,level.replace('Easy','Default'))]
...
class Achievement(object):
    def __init__(self,name,iconName,iconColor,level,award,hardModeOnly=False):
        self._name = name
    ...
    def _getConfig(self):
        if not 'Achievements' in bs.getConfig(): bs.getConfig()['Achievements'] = {}
    ...
    def setComplete(self,complete=True,announce=True,sound=True,writeConfig=True):
        config = self._getConfig()
    ...
    def getDisplayString(self):
        try:
            ...
            n = ''
            bs.printException()
        return bs.Lstr(resource='achievements.'+self._name+'.name',subs=[('${LEVEL}',n)])
    ...
    def getDescription(self):
        if 'description' in bsUtils._getResource('achievements')[self._name]:
            ...
            return bs.Lstr(resource='achievements.'+self._name+'.descriptionFull')
    
    def getDescriptionComplete(self):
        if 'descriptionComplete' in bsUtils._getResource('achievements')[self._name]:
            return bs.Lstr(resource='achievements.'+self._name+'.descriptionComplete')
        else:
            return bs.Lstr(resource='achievements.'+self._name+'.descriptionFullComplete')

    def getDescriptionFull(self):
        return bs.Lstr(resource='achievements.'+self._name+'.descriptionFull',subs=[('${LEVEL}',bs.Lstr(translate=['coopLevelNames', gAchLevelNames.get(self._name,'?')]))])

    def getDescriptionFullComplete(self):
        return bs.Lstr(resource='achievements.'+self._name+'.descriptionFullComplete',subs=[('${LEVEL}',bs.Lstr(translate=['coopLevelNames', gAchLevelNames.get(self._name,'?')]))])

    def getAwardTicketValue(self,includeProBonus=False):
        'Return the ticket award value for this achievement'
        return bsInternal._getAccountMiscReadVal('achAward.'+self._name,self._award)*_getAchMult(includeProBonus)

    def getPowerRankingValue(self):
        'Return the power-ranking award value for this achievement'
        return bsInternal._getAccountMiscReadVal('achLeaguePoints.'+self._name,self._award)
    
    def createDisplay(self,x,y,delay,outDelay=None,color=None,style='postGame'):
        ...
        if inMainMenu:
            ...
            try:
                hmo = (self._hardModeOnly and bs.getSession()._campaignInfo['campaign'] == 'Easy')
            except Exception:
        ...
        if inGameColors:
            ...
            txt2S = 0.62
            txt2MaxW = 400
            objs.append(bsUtils.Text(self.getDescriptionFull() if inMainMenu else self.getDescription(),
                                     hostOnly=True,
                                     maxWidth=txt2MaxW,
                                     position=(x,y-14),
                                     transition='fadeIn',
            ...
            # ticket-award
            awardX = -100
            objs.append(bsUtils.Text(bs.getSpecialChar('ticket'),
                                     hostOnly=True,
                                     ...
                                     transitionDelay=delay+50,
                                     transitionOutDelay=outDelayFin).autoRetain())
            objs.append(bsUtils.Text('+'+str(self.getAwardTicketValue()),
                                     hostOnly=True,
            ...
            objs.append(bsUtils.Image(self.getIconTexture(complete),
                                      ...
                                      scale=(40,40)).autoRetain())
            if complete:
                objs.append(bsUtils.Image(bs.getTexture('achievementOutline'),
                                          hostOnly=True,
                ...
                if not complete:
                    ...
                    objs.append(bsUtils.Text(bs.getSpecialChar('ticket'),
                                             ...
                                             hAlign='center',
                                             vAlign='center',
                                             color=(1,1,1,0.4) if complete else (1,1,1,(0.1 if hmo else 0.2)),
                                             transitionDelay=delay+50,
                    ...
                    objs.append(bsUtils.Text('+'+str(self.getAwardTicketValue()),
                                             ...
                                             hAlign='center',
                                             vAlign='center',
                                             color=((0.8,0.93,0.8,1.0) if complete else (0.6,0.6,0.6,(0.2 if hmo else 0.4))),
                                             transitionDelay=delay+50,
                                             transitionOutDelay=None).autoRetain())
            ...
            objs.append(bsUtils.Text(self.getDisplayString(),
                                     hostOnly=True,
                                     maxWidth=300,
                                     position=(x,y+2),
                                     transition='inRight',
                                     ...
                                     transitionDelay=delay+50,
                                     transitionOutDelay=None).autoRetain())
            objs.append(bsUtils.Text(self.getDescriptionComplete() if complete else self.getDescription(),
                                     hostOnly=True,
                                     maxWidth=400,
                                     position=(x,y-14),
                                     transition='inRight',
                                     ...
                                     scale=0.62,
                                     flatness=1.0,
                                     color=((0.6,0.6,0.6,1.0) if complete else (0.6,0.6,0.6,(0.2 if hmo else 0.4))),
                                     transitionDelay=delay+100,
                                     transitionOutDelay=None).autoRetain())
        return objs
    ...
    def showCompletionBanner(self,sound=True):
        ...
        else: bs.gameTimer(500,bs.Call(bs.playSound,bs.getSound('ding'),hostOnly=True))
        ...
        bsUtils.animate(c,'input0',{inTime:0, inTime+400:30, inTime+500:40, inTime+600:30, inTime+2000:0})
        bsUtils.animate(c,'input1',{inTime:0, inTime+400:200, inTime+500:500, inTime+600:200, inTime+2000:0})
        ...
        keys = {inTime:1.0*color[0], inTime+400:1.5*color[0], inTime+500:6.0*color[0], inTime+600:1.5*color[0], inTime+2000:1.0*color[0]}
        bsUtils.animate(c,'input0',keys)
        keys = {inTime:1.0*color[1], inTime+400:1.5*color[1], inTime+500:6.0*color[1], inTime+600:1.5*color[1], inTime+2000:1.0*color[1]}
        bsUtils.animate(c,'input1',keys)
        keys = {inTime:1.0*color[2], inTime+400:1.5*color[2], inTime+500:6.0*color[2], inTime+600:1.5*color[2], inTime+2000:1.0*color[2]}
        ...
        obj = bsUtils.Text(bs.Lstr(value='${A}:',subs=[('${A}',bs.Lstr(resource='achievementText'))]),
        ...
        for obj in objs: bs.gameTimer(outTime+1000,bs.WeakCall(obj.handleMessage,bs.DieMessage()))

# 5
gAchievements.append(Achievement('In Control','achievementInControl',(1,1,1),'',5))
# 15
gAchievements.append(Achievement('Sharing is Caring','achievementSharingIsCaring',(1,1,1),'',15))
# 10
gAchievements.append(Achievement('Dual Wielding','achievementDualWielding',(1,1,1),'',10))

# 10
gAchievements.append(Achievement('Free Loader','achievementFreeLoader',(1,1,1),'',10))
# 20
gAchievements.append(Achievement('Team Player','achievementTeamPlayer',(1,1,1),'',20))

#5
gAchievements.append(Achievement('Onslaught Training Victory','achievementOnslaught',(1,1,1),'Default:Onslaught Training',5))
#5
gAchievements.append(Achievement('Off You Go Then','achievementOffYouGo',(1,1.1,1.3),'Default:Onslaught Training',5))
#10
gAchievements.append(Achievement('Boxer','achievementBoxer',(1,0.6,0.6),'Default:Onslaught Training',10,hardModeOnly=True))

#10
gAchievements.append(Achievement('Rookie Onslaught Victory','achievementOnslaught',(0.5,1.4,0.6),'Default:Rookie Onslaught',10))
#10
gAchievements.append(Achievement('Mine Games','achievementMine',(1,1,1.4),'Default:Rookie Onslaught',10))
#15
gAchievements.append(Achievement('Flawless Victory','achievementFlawlessVictory',(1,1,1),'Default:Rookie Onslaught',15,hardModeOnly=True))

#10
gAchievements.append(Achievement('Rookie Football Victory','achievementFootballVictory',(1.0,1,0.6),'Default:Rookie Football',10))
#10
gAchievements.append(Achievement('Super Punch','achievementSuperPunch',(1,1,1.8),'Default:Rookie Football',10))
#15
gAchievements.append(Achievement('Rookie Football Shutout','achievementFootballShutout',(1,1,1),'Default:Rookie Football',15,hardModeOnly=True))

#15
gAchievements.append(Achievement('Pro Onslaught Victory','achievementOnslaught',(0.3,1,2.0),'Default:Pro Onslaught',15))
#15
gAchievements.append(Achievement('Boom Goes the Dynamite','achievementTNT',(1.4,1.2,0.8),'Default:Pro Onslaught',15))
#20
gAchievements.append(Achievement('Pro Boxer','achievementBoxer',(2,2,0),'Default:Pro Onslaught',20,hardModeOnly=True))

#15
gAchievements.append(Achievement('Pro Football Victory','achievementFootballVictory',(1.3,1.3,2.0),'Default:Pro Football',15))
#15
gAchievements.append(Achievement('Super Mega Punch','achievementSuperPunch',(2,1,0.6),'Default:Pro Football',15))
#20
gAchievements.append(Achievement('Pro Football Shutout','achievementFootballShutout',(0.7,0.7,2.0),'Default:Pro Football',20,hardModeOnly=True))

#15
gAchievements.append(Achievement('Pro Runaround Victory','achievementRunaround',(1,1,1),'Default:Pro Runaround',15))
#20
gAchievements.append(Achievement('Precision Bombing','achievementCrossHair',(1,1,1.3),'Default:Pro Runaround',20,hardModeOnly=True))
#25
gAchievements.append(Achievement('The Wall','achievementWall',(1,0.7,0.7),'Default:Pro Runaround',25,hardModeOnly=True))

#30
gAchievements.append(Achievement('Uber Onslaught Victory','achievementOnslaught',(2,2,1),'Default:Uber Onslaught',30))
#30
gAchievements.append(Achievement('Gold Miner','achievementMine',(2,1.6,0.2),'Default:Uber Onslaught',30,hardModeOnly=True))
#30
gAchievements.append(Achievement('TNT Terror','achievementTNT',(2,1.8,0.3),'Default:Uber Onslaught',30,hardModeOnly=True))

#30
gAchievements.append(Achievement('Uber Football Victory','achievementFootballVictory',(1.8,1.4,0.3),'Default:Uber Football',30))
#30
gAchievements.append(Achievement('Got the Moves','achievementGotTheMoves',(2,1,0),'Default:Uber Football',30,hardModeOnly=True))
#40
gAchievements.append(Achievement('Uber Football Shutout','achievementFootballShutout',(2,2,0),'Default:Uber Football',40,hardModeOnly=True))

#30
gAchievements.append(Achievement('Uber Runaround Victory','achievementRunaround',(1.5,1.2,0.2),'Default:Uber Runaround',30))
#40
gAchievements.append(Achievement('The Great Wall','achievementWall',(2,1.7,0.4),'Default:Uber Runaround',40,hardModeOnly=True))
#40
gAchievements.append(Achievement('Stayin\' Alive','achievementStayinAlive',(2,2,1),'Default:Uber Runaround',40,hardModeOnly=True))

#20
gAchievements.append(Achievement('Last Stand Master','achievementMedalSmall',(2,1.5,0.3),'Default:The Last Stand',20,hardModeOnly=True))
#40
gAchievements.append(Achievement('Last Stand Wizard','achievementMedalMedium',(2,1.5,0.3),'Default:The Last Stand',40,hardModeOnly=True))
#60
gAchievements.append(Achievement('Last Stand God','achievementMedalLarge',(2,1.5,0.3),'Default:The Last Stand',60,hardModeOnly=True))

#5
gAchievements.append(Achievement('Onslaught Master','achievementMedalSmall',(0.7,1,0.7),'Challenges:Infinite Onslaught',5))
#15
gAchievements.append(Achievement('Onslaught Wizard','achievementMedalMedium',(0.7,1.0,0.7),'Challenges:Infinite Onslaught',15))
#30
gAchievements.append(Achievement('Onslaught God','achievementMedalLarge',(0.7,1.0,0.7),'Challenges:Infinite Onslaught',30))

#5
gAchievements.append(Achievement('Runaround Master','achievementMedalSmall',(1.0,1.0,1.2),'Challenges:Infinite Runaround',5))
#15
