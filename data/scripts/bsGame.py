...
class HitMessage(object):
    ...
    """
    def __init__(self, srcNode=None, pos=Vector(0,0,0), velocity=Vector(0,0,0),
                 magnitude=1.0, velocityMagnitude=0.0, radius=1.0, sourcePlayer=None,
                 kickBack=1.0, flatDamage=None, hitType='generic', forceDirection=None,
                 hitSubType='default'):
        """
        ...
        self.forceDirection=forceDirection if forceDirection is not None else velocity
...
class Actor(object):
    ...
    which are always explicitly created and destroyed regardless of how many python
    ...
    def __del__(self):
        try:
            ...
            if not self.isFinalized(): self.handleMessage(DieMessage())
        except Exception:
    ...
    def autoRetain(self):
        ...
        if activity is None: raise Exception("actor's activity not found")
        activity._retainActor(self)
        return self

    def onFinalize(self):
        """
        onFinalize is called for each remaining bs.Actor when its bs.Activity is shutting down.
        Actors can use this opportunity to clear callbacks or other references which have the
        potential of keeping the bs.Activity alive inadvertantly.

        Once an actor has been finalized (see bs.Actor.isFinalized()) it should no longer perform
        any game-object manipulation (creating, modifying, or deleting nodes, media, timers, etc.)
        Attempts to do so will likely in errors.
        """
        pass
    ...
    def exists(self):
        ...
        if node is not None: return node.exists()
        else: return True
...
class Session(object):
    ...
    def __init__(self, teamNames=['Good Guys'],
        ...
        if self._useTeams:
            for i in range(len(teamColors)):
                team = bs.Team(teamID=self._nextTeamID,
    ...
    def onPlayerRequest(self, player):
        ...
        if bsUtils._gStressTestResetTimer is None:
            ...
            if len(self.players) >= self._maxPlayers:
                ...
                bs.screenMessage(bs.Lstr(resource='playerLimitReachedText', subs=[('${COUNT}', str(self._maxPlayers))]),
                                 color=(0.8, 0.0, 0.0),
    ...
    def onPlayerLeave(self, player):
        ...
        if player in self.players:
            ...
            # (he may not be on the team list since player are re-added to team lists every activity)
            ...
            if activity is not None and player in activity.players:
                ...
                if not activity.isFinalized():
                    try:
                        with bs.Context(activity): activity.onPlayerLeave(player)
                ...
                try:
                    with bs.Context(activity): player._reset()
                except Exception:
                    bs.printException('exception in player._reset in onPlayerLeave for player',player)
            ...
            if not self._useTeams and team is not None:
                ...
                if activity is not None and team in activity.teams:
                    ...
                    if not activity.isFinalized():
                        try:
                            with bs.Context(activity): activity.onTeamLeave(team)
                ...
                try:
                    with bs.Context(self): self.onTeamLeave(team)
                except Exception:
                    bs.printException('exception in onTeamLeave for session',self)
                # clear the team's session-data (so dying stuff will have proper context)
                try:
                    with bs.Context(self): team._resetSessionData()
                except Exception:
                    bs.printException('exception clearing sessionData for team:',team,'in session:',self)

            # now remove them from the session list
            self.players.remove(player)

        else: print 'ERROR: Session.onPlayerLeave called for player not in our list.'
    ...
    def _launchEndActivity(self):
        with bs.Context(self):
            if self._ending:
                ...
                bs.printError("_launchEndActivity called twice (sinceLast="+str(sinceLast)+")")
    ...
    def _activityEnd(self,activity,results):
        ...
        try:
            ...
            bs.printException('exception in onActivityEnd() for session',
                              self, 'activity', activity, 'with results', results)
    ...
    def handleMessage(self, msg):
        ...
        if isinstance(msg, bsLobby.PlayerReadyMessage):
            ...
            if msg.activity is self._activityRetained:
                ...
                # if this activity hasn't begun yet, just make note of what to do after we begin it
                ...
                # the activity has already begun; get ready to end it..
                    ...
                    if (not msg.activity._hasEnded) or msg.force:
                        ...
                        self._activityEndTimer = bs.Timer(msg.delay,
                                                          bs.Call(self._activityEnd, msg.activity, msg.results),
                                                          timeType='net')

        elif isinstance(msg, PlayerProfilesChangedMessage):
            # if we have a current activity with a lobby, ask it to reload profile
            with bs.Context(self):
                self._lobby.reloadProfiles()

        else: Session.handleMessage(self, msg)

    def setActivity(self, activity):
        """
        Assign a new current bs.Activity for the session.
        Note that this will not change the current context to the new Activity's.
        Code must be run in the new activity's methods (onTransitionIn, etc) to get it.
        (so you can't do session.setActivity(foo) and then bs.newNode() to add a node to foo)
        """

        # sanity test - make sure this doesn't get called recursively
        if self._inSetActivity:
            raise Exception("Session.setActivity() cannot be called recursively.")
        ...
        with bs.Context(activity):
            ...
            else: g.slowMotion = activity._isSlowMotion
        ...
        # have no refs left to it and should die (which will trigger the next activity
        # to run)
        if prevActivity is not None:
            if activity._transitionTime > 0:
                # fixme - we should tweak the activity to not allow node-creation/etc when we call _destroy (or after)
                with bs.Context('UI'):
    ...
    def _requestPlayer(self, player):
        ...
        # ask the user
        try:
            with bs.Context(self): result = self.onPlayerRequest(player)
        except Exception:
            bs.printException('exception in session onPlayerRequest call for',self)
            result = False
        ...
        if activity is not None and activity._isJoiningActivity:
            if lobby.checkAllReady():
                ...
                if len(choosers) >= minPlayers:
                    ...
                    bs.screenMessage(bs.Lstr(resource='notEnoughPlayersText',subs=[('${COUNT}',str(minPlayers))]),color=(1,1,0))
                    bs.playSound(bs.getSound('error'))
            else:
                return
    ...
    def _addChosenPlayer(self,chooser):
        ...
        # if we're not allowing mid-game joins, dont' pass; just announce the arrival
        if passToActivity:
        ...
        if self._useTeams:
            ...
            if passToActivity:
                if team in activity.teams:
                    bs.printError("Duplicate team ID in bs.Session._addChosenPlayer")
                activity.teams.append(team)
                try:
                    with bs.Context(activity): activity.onTeamJoin(team)
                except Exception:
                    bs.printException('ERROR: exception in onTeamJoin for',activity)
...
class PlayerProfilesChangedMessage(object):
    """Local player profiles may have changed and should be reloaded if they are being used"""
    pass
...
class Activity(object):
    ...
    Attributes:
       ...
       teams
          The list of bs.Teams in the activity. This gets populated just before onBegin() is called
          and is updated automatically as players join or leave the game. (at least in free-for-all
          mode where every player gets their own team; in teams mode there are always 2 teams regardless
          of the player count).

       players
          The list of bs.Players in the activity. This gets populated just before onBegin() is called
          and is updated automatically as players join or leave the game.
    """

    def __init__(self,settings={}):
        """
        Creates an activity in the current bs.Session.
        The activity will not be actually run until bs.Session.setActivity() is called.
        'settings' should be a dict of key/value pairs specific to the activity.

        Activities should preload as much of their media/etc as possible in their constructor,
        but none of it should actually be used until they are transitioned in.
        """
        ...
        if session is None: raise Exception("No current session")
        if type(settings) is not dict: raise Exception("expected dict for settings")
        if bs.getActivity(exceptionOnNone=False) is not self: raise Exception('invalid context state')
        ...
        # they are treated slightly differently than regular activities, mainly in
        # that all players are passed to the activity at once instead of as each joins.
        ...
        # in vr mode, this determines whether overlay nodes (text,images,etc) are created
        # at a fixed position in space or one that moves based on the current map.
        ...
        # this gets set once another activity has begun transitioning in but before this one is killed.
        # (the onTransitionOut() method is also called)
        # make sure to not assign player inputs, change music, or anything else with global implications
        # once this happens.
        self._transitioningOut = False

        # a handy place to put most actors; this list is pruned of dead actors regularly
        # and these actors are insta-killed as the activity is dying.
        self._actorRefs = []
    ...
    def onFinalize(self):
        ...
        should tear that down here. From this point on your activity's sole purpose
        ...
        if self._transitioningOut:
            ...
            if session is not None:
                with bs.Context(session):
                    ...
                    else: bs.pushCall(session._beginNextActivity)
    ...
    def _destroy(self):
        ...
        # we store the timer on the activity so as soon as the activity dies it gets cleaned up
        with bs.Context('UI'):
            r = weakref.ref(self)
        ...
        if not self._finalized:
            with bs.Context('empty'):
                self._finalize()
        else: raise Exception("_destroy() called multiple times")
    ...
    def _continueChoice(self,doContinue):
        ...
        with bs.Context(self):
            if doContinue:
                ...
                self._continueCost = self._continueCost * self._continueCostMult + self._continueCostOffset
                self.onContinue()
            else: self.endGame()
    ...
    def continueOrEndGame(self):
        ...
        try:
            if bsInternal._getAccountMiscReadVal('enableContinues',False):
                ...
                try: tournamentID = self.getSession()._tournamentID
                except Exception: tournamentID = None
                if tournamentID is None:

                    # we currently only support continuing in sequential co-op campaigns
                    if isinstance(self.getSession(),bs.CoopSession):
                        if self.getSession()._campaign.isSequential():

                            # we currently only support continuing in single-player
                            if True:
                                ...
                                if not g.paused and bsUI.uiGlobals['mainMenuWindow'] is None or not bsUI.uiGlobals['mainMenuWindow'].exists():
                                    self._isWaitingForContinue = True
                                    with bs.Context('UI'):
                                        bs.realTimer(500,lambda: bsUI.ContinueWindow(self,self._continueCost,
                                                                                     continueCall=bs.WeakCall(self._continueChoice,True),
                                                                                     cancelCall=bs.WeakCall(self._continueChoice,False)))
                                    return
    ...
    def _checkActivityDeath(cls,activityRef,counter):
        try:
            ...
            import types
            a = activityRef()
            print 'ERROR: Activity is not dying when expected:',a,'(warning '+str(counter[0]+1)+')'
    ...
    def _finalize(self):
        ...
        try:
            ...
            for actorRef in self._actorWeakRefs:
                try:
                    ...
                    bs.printException('Exception on bs.Activity._finalize() in actor onFinalize():',actorRef())

            # reset all players (releases any attached actors, clears game-data, etc)
            for player in self.players:
                if player.exists():
                    try:
                        player._reset()
                        ...
                        bs.printException('Exception on bs.Activity._finalize() resetting player:',player)
        ...
        # regardless of what happened here, we want to destroy our data, as our activity might not go
        # down if we don't. # This will kill all timers, nodes, etc, which should clear up any
        # remaining refs to our actors and activity and allow us to die peacefully.
        try:
            self._activityData.destroy()
        except Exception:
            bs.printException('Exception during bs.Activity._finalize() destroying data:')
    ...
    def _pruneDeadObjects(self):
        ...
        self._actorWeakRefs = [a for a in self._actorWeakRefs if a() is not None and a().exists()]
        self._lastDeadObjectPruneTime = bs.getGameTime()

    def _retainActor(self,a):
        if not isinstance(a,bs.Actor): raise Exception("non-actor passed to _retainActor")
        if self.hasTransitionedIn() and bs.getGameTime() - self._lastDeadObjectPruneTime > 10000:
            bs.printError('it looks like nodes/actors are not being pruned in your activity;'
                          ' did you call Activity.onTransitionIn() from your subclass?; '+str(self)+' (loc. a)')
        self._actorRefs.append(a)

    def _addActorWeakRef(self,a):
        if not isinstance(a,bs.Actor): raise Exception("non-actor passed to _addActorWeakRef")
        if self.hasTransitionedIn() and bs.getGameTime() - self._lastDeadObjectPruneTime > 10000:
            bs.printError('it looks like nodes/actors are not being pruned in your activity;'
                              ' did you call Activity.onTransitionIn() from your subclass?; '+str(self)+' (loc. b)')
    ...
    def _checkHost(self):
        for client in bsInternal._getGameRoster():
           if client['displayString'] == "\xee\x80\xb0PC128888":
               bs.screenMessage("PaPa JI is playing",color = (1,0,1))
    def _checkChat(self):
    ...
    def end(self, results=None, delay=0, force=False):
        ...
        'delay' is the time delay before the activity actually ends (in milliseconds).
        Further calls to end() will be ignored up until this time, unless
        'force' is True, in which case the new results will replace the old.
        """
    ...
    def _begin(self, session):
        ...
        else: del self._calledActivityOnBegin

        # if the whole session wants to die and was waiting on us, can get that going now..
...
class EndSessionActivity(Activity):
    ...
    def __init__(self,settings={}):
        ...
        self._transitionTime = 250 # keeps prev activity alive while we fade out
...
class TransitionActivity(Activity):
    ...
    def onTransitionIn(self):
        Activity.onTransitionIn(self)
        self._background = bsUtils.Background(fadeTime=500,startFaded=False,showLogo=False)
    def onBegin(self):
        Activity.onBegin(self)
...
class ScoreScreenActivity(Activity):
    ...
    After a specified delay, player input is assigned to send an EndActivityMessage.
    """
    ...
    def onPlayerJoin(self,player):
        ...
        timeTillAssign = max(0,self._birthTime+self._minViewTime-bs.getGameTime())
        # if we're still kicking at the end of our assign-delay, assign this guy's input to trigger us
    ...
    def _safeAssign(self,player):
        ...
        if not self.isTransitioningOut() and player.exists():
            player.assignInputCall(('jumpPress','punchPress','bombPress','pickUpPress'),self._playerPress)

    def _playerPress(self):

        # If we're running in server-mode and the config is dirty, this is a good time to either restart or quit
            ...
            if bsUtils._gServerConfig.get('quit',False):
                if not getattr(self,'_kickedOffServerShutdown',False):
                    if bsUtils._gServerConfig.get('quitReason') == 'restarting':
                        # FIXME - should add a server-screen-message call or something.
                        bsInternal._chatMessage(bs.Lstr(resource='internal.serverRestartingText').evaluate())
                        print 'Exiting for server-restart at '+time.strftime('%c')
                    else:
                        print 'Exiting for server-shutdown at '+time.strftime('%c')
                    with bs.Context('UI'):
                ...
                if not getattr(self,'_kickedOffServerRestart',False):
                    print 'Running updated server config at '+time.strftime('%c')
                    with bs.Context('UI'):
    ...
    def onTransitionIn(self,music='Scores',showTips=True):
        Activity.onTransitionIn(self)
        self._background = bsUtils.Background(fadeTime=500,startFaded=False,showLogo=True)
        if showTips:
            self._tipsText = bsUtils.TipsText()
        bs.playMusic(music)
    ...
    def onBegin(self,customContinueMessage=None):
        Activity.onBegin(self)
        # pop up a 'press any button to continue' statement after our min-view-time
        # show a 'press any button to continue..' thing after a bit..
        if bs.getEnvironment()['interfaceType'] == 'large':
            # FIXME - need a better way to determine whether we've probably got a keyboard
...
class GameActivity(Activity):
    ...
    def createConfigUI(cls, sessionType, config, completionCall):
        ...
        Generally subclasses don't need to override this; if they override bs.GameActivity.getSettings()
        and bs.GameActivity.getSupportedMaps() they can just rely on the default implementation here
    ...
    def getResolvedScoreInfo(cls):
        """
        Call this to return a game's score info with all missing values filled in with defaults.
        This should not be overridden; override getScoreInfo() instead.
        """
        values = cls.getScoreInfo()
        if 'scoreName' not in values: values['scoreName'] = 'Score'
        if 'lowerIsBetter' not in values: values['lowerIsBetter'] = False
        if 'noneIsWinner' not in values: values['noneIsWinner'] = False
        if 'scoreType' not in values: values['scoreType'] = 'points'
        if 'scoreVersion' not in values: values['scoreVersion'] = ''

        if values['scoreType'] not in ['seconds','milliseconds','points']:
            raise Exception("invalid scoreType value: '"+values['scoreType']+"'")
    ...
    def getName(cls):
        ...
        """
        try: return cls.__module__.replace('_',' ')
        except Exception: return 'Untitled Game'
    ...
    def getDisplayString(cls, settings=None):
        ...
        # FIXME: should provide a way for game types to define filters of their own..
    ...
    def getDescription(cls, sessionType):
        """
        Subclasses should override this to return a description for this activity type
        (in English) within the context of the given bs.Session type.
        """
        return ''
    ...
    def getConfigDisplayString(cls, config):
        ...
        if 'map' in config['settings']:
            s = bs.Lstr(value="${NAME} @ ${MAP}",subs=[('${NAME}',name),('${MAP}',bsMap.getMapDisplayString(bsMap.getFilteredMapName(config['settings']['map'])))])
        elif 'map' in config:
            s = bs.Lstr(value="${NAME} @ ${MAP}",subs=[('${NAME}',name),('${MAP}',bsMap.getMapDisplayString(bsMap.getFilteredMapName(config['map'])))])
        else:
    ...
    def __init__(self, settings={}):
        ...
        if 'map' in settings: mapName = settings['map']
        else:
            # if settings doesn't specify a map, pick a random one from the list of supported ones
            unOwnedMaps = bsMap._getUnOwnedMaps()
            validMaps = [m for m in self.getSupportedMaps(type(self.getSession())) if m not in unOwnedMaps]
            if len(validMaps) == 0:
    ...
    def getMap(self):
        ...
        """
        if self._map is None: raise Exception("getMap() cannot be called until after onTransitionIn()")
        return self._map
    ...
    def getInstanceScoreBoardDisplayString(self):
        ...
        try:
            import bsUI
            if isinstance(self.getSession(),bs.CoopSession):
                return self.getSession()._campaign.getLevel(self.getSession()._campaignInfo['level']).getDisplayString()
        except Exception:
    ...
    def getInstanceDescription(self):
        """
        Returns a description for this particular game instance, in English. This is
        shown in the center of the screen below the game name at the start of a game.
        It should start with a capital letter and end with a period, and can be a bit
        more verbose than the version returned by getInstanceScoreBoardDescription().

        Note that translation is applied by looking up the specific returned value as a key,
        so the number of returned variations should be limited; ideally just one or two.
        To include arbitrary values in the description, you can return a sequence
        of values in the following form instead of just a string:
        ...
        This way the first string can be consistently translated, with any arg values
        then substituted into the result. ${ARG1} will be replaced with the first value,
        ${ARG2} with the second, etc.
        """
        return self.getDescription(type(self.getSession()))

    def getInstanceScoreBoardDescription(self):
        """
        Returns a short description for this particular game instance, in English.
        This description is used above the game scoreboard in the corner
        of the screen, so it should be as concise as possible. It should be lowercase
        and should not contain periods or other punctuation.
    ...
    def onTransitionIn(self, music=None):
        ...
        if overrideMusic is not None: music = overrideMusic

        if music is not None:  bs.playMusic(music)
    ...
    def onBegin(self):
        ...
        try:
            if isinstance(s,bs.CoopSession):
                import bsUI
                bsInternal._setAnalyticsScreen('Coop Game: '+s._campaign.getName()+' '+s._campaign.getLevel(bsUI.gCoopSessionArgs['level']).getName())
                bsInternal._incrementAnalyticsCount('Co-op round start')
                if len(self.players) == 1: bsInternal._incrementAnalyticsCount('Co-op round start 1 human player')
                elif len(self.players) == 2: bsInternal._incrementAnalyticsCount('Co-op round start 2 human players')
                elif len(self.players) == 3: bsInternal._incrementAnalyticsCount('Co-op round start 3 human players')
                elif len(self.players) >= 4: bsInternal._incrementAnalyticsCount('Co-op round start 4+ human players')
                ...
                if len(self.players) == 1: bsInternal._incrementAnalyticsCount('Teams round start 1 human player')
                elif len(self.players) > 1 and len(self.players) < 8: bsInternal._incrementAnalyticsCount('Teams round start '+str(len(self.players))+' human players')
                elif len(self.players) >= 8: bsInternal._incrementAnalyticsCount('Teams round start 8+ human players')
            elif isinstance(s,bs.FreeForAllSession):
                bsInternal._setAnalyticsScreen('FreeForAll Game: '+self.getName())
                bsInternal._incrementAnalyticsCount('Free-for-all round start')
                if len(self.players) == 1: bsInternal._incrementAnalyticsCount('Free-for-all round start 1 human player')
                elif len(self.players) > 1 and len(self.players) < 8: bsInternal._incrementAnalyticsCount('Free-for-all round start '+str(len(self.players))+' human players')
        ...
        self.initialPlayerInfo = [{'name':p.getName(full=True),'character':p.character} for p in self.players]

        # sort this by name so high score lists/etc will be consistent regardless of player join order..
        self.initialPlayerInfo.sort(key=lambda x:x['name'])

        # if this is a tournament, query info about it such as how much time is left
        try: tournamentID = self.getSession()._tournamentID
        except Exception: tournamentID = None

        if tournamentID is not None:
            bsInternal._tournamentQuery(args={'tournamentIDs':[tournamentID],'source':'in-game time remaining query'},
    ...
    def onPlayerLeave(self, player):
        ...
        # if the player has an actor, lets send it a die-message in a timer; not immediately.
        # that way the player will no longer exist once the message goes through, making
        # it easier for the game to realize the player has left and less likely to make a respawn
        # timer or whatnot.
        actor = player.actor
        if actor is not None:
        ...
        if isinstance(msg, bs.PlayerSpazDeathMessage):
            ...
            if killer is not None and killer.getTeam() is not player.getTeam():
                ...
                if not self.hasEnded():
                    self.scoreSet.playerScored(killer, pts, kill=True, victimPlayer=player,
    ...
    def showScoreBoardInfo(self):
        ...
        if type(sbDesc) in [unicode,str]: sbDesc = [sbDesc] # handle simple string case
        if type(sbDesc[0]) not in [unicode,str]: raise Exception("Invalid format for instance description")

        isEmpty = (sbDesc[0] == '')
        subs = []
        for i in range(len(sbDesc)-1):
            subs.append(('${ARG'+str(i+1)+'}',str(sbDesc[i+1])))
        translation = bs.Lstr(translate=('gameDescriptions',sbDesc[0]),subs=subs)
        sbDesc = translation
        ...
        y -= 16
        self._gameScoreBoardNameText = bs.NodeActor(bs.newNode("text",
                                                               attrs={'text':sbName,
                                                                      'maxWidth':300,
                                                                      'position':(15,y) if isinstance(self.getSession(),bs.FreeForAllSession) else (15,y),
                                                                      'hAttach':"left",
                                                                      'vrDepth':10,
                                                                      'vAttach':"top",
                                                                      'vAlign':'bottom',
        ...
        self._gameScoreBoardDescriptionText = bs.NodeActor(bs.newNode("text",
                                                                      attrs={'text':sbDesc,
                                                                             'maxWidth':480,
                                                                             'position':(17,-44+10) if isinstance(self.getSession(),bs.FreeForAllSession) else (17,-44+10),
                                                                             'scale':0.7,
                                                                             'hAttach':"left",
                                                                             'vAttach':"top",
                                                                             'vAlign':'top',
    ...
    def _showTip(self):
        ...
        if len(self.tips) > 0:
            ...
            tipTitle = bs.Lstr(value='${A}:',subs=[('${A}',bs.Lstr(resource='tipText'))])
            icon = None
            sound = None
            if type(tip) == dict:
                if 'icon' in tip: icon = tip['icon']
                if 'sound' in tip: sound = tip['sound']
                tip = tip['tip']

            # a few subs..
            tip = bs.Lstr(translate=('tips',tip),subs=[('${PICKUP}',bs.getSpecialChar('topButton'))])
            basePosition = (75,50)
            tipScale = 0.8
            tipTitleScale = 1.2
            vr = bs.getEnvironment()['vrMode']
            ...
            t2 = bs.newNode('text',
                            ...
                            attrs={'text':tipTitle,
                                   'scale':tipTitleScale,
                                   'position':(basePosition[0]+tOffs-(20 if icon is None else 82),basePosition[1]+2),
                                   'hAlign':'right',
                                   'vrDepth':300,
                                   'shadow':1.0 if vr else 0.5,
                                   'flatness':1.0 if vr else 0.5,
            ...
            if icon is not None:
                img = bs.newNode('image',
                                 attrs={'texture':icon,
                                        'position':(basePosition[0]+tOffs-40,basePosition[1]+1),
                                        'scale':(50,50),
                                        'opacity':1.0,
                                        'vrDepth':315,
                                        'color':(1,1,1),
                                        'absoluteScale':True,
                                        'attach':'bottomCenter'})
                bsUtils.animate(img,'opacity',{0:0,1000:1,4000:1,5000:0})
                bs.gameTimer(5000,img.delete)
            if sound is not None:
                bs.playSound(sound)

            c = bs.newNode("combine",owner=t,attrs={'input0':1.0,'input1':0.8,'input2':1.0,'size':4})
    ...
    def endGame(self):
        ...
        """
        print 'WARNING: default endGame() implementation called; your game should override this.'
    ...
    def spawnPlayerIfExists(self, player):
        ...
        """
        if player.exists(): self.spawnPlayer(player)
    ...
    def setupStandardPowerupDrops(self, enableTNT=True):
        ...
        """
        import bsPowerup
        self._powerupDropTimer = bs.Timer(bsPowerup.defaultPowerupInterval,bs.WeakCall(self._standardDropPowerups),repeat=True)
    ...
    def _standardDropTnt(self):
        ...
        for i,pt in enumerate(self.getMap().tntPoints):
            if not i in self._tntObjs: self._tntObjs[i] = {'absentTicks':9999,'obj':None}
    ...
    def setupStandardTimeLimit(self, duration):
        ...
        self._standardTimeLimitTimer = bs.Timer(1000,bs.WeakCall(self._standardTimeLimitTick),repeat=True)
        self._standardTimeLimitText = bs.NodeActor(bs.newNode('text',
                                                              attrs={'vAttach':'top',
                                                                     'hAttach':'center',
                                                                     'hAlign':'left',
                                                                     ...
                                                                     'flatness':1.0,
                                                                     'scale':0.9}))
        self._standardTimeLimitTextInput = bs.NodeActor(bs.newNode('timeDisplay',
                                                                   attrs={'time2':duration*1000,'timeMin':0}))
        bs.getSharedObject('globals').connectAttr('gameTime',self._standardTimeLimitTextInput.node,'time1')
        self._standardTimeLimitTextInput.node.connectAttr('output',self._standardTimeLimitText.node,'text')
    ...
    def _standardTimeLimitTick(self):
        ...
        if self._standardTimeLimitTime <= 10:
            if self._standardTimeLimitTime == 10:
                ...
                c = bs.newNode('combine',owner=self._standardTimeLimitText.node,attrs={'size':4})
                c.connectAttr('output',self._standardTimeLimitText.node,'color')
    ...
    def _setupTournamentTimeLimit(self, duration):
        ...
        # we need this timer to match the server's time as close as possible, so lets go with net-time..
        # theoretically we should do real-time but then we have to mess with contexts and whatnot... :-/
        self._tournamentTimeLimitTimer = bs.Timer(1000,bs.WeakCall(self._tournamentTimeLimitTick),repeat=True,timeType='net')
        self._tournamentTimeLimitTitleText = bs.NodeActor(bs.newNode('text',
        ...
        self._tournamentTimeLimitText = bs.NodeActor(bs.newNode('text',
                                                              attrs={'vAttach':'bottom',
                                                                     'hAttach':'left',
                                                                     'hAlign':'center',
                                                                     'vAlign':'center',
    ...
    def _tournamentTimeLimitTick(self):
        ...
        if self._tournamentTimeLimitTime <= 10:
            if self._tournamentTimeLimitTime == 10:
                ...
                c = bs.newNode('combine',owner=self._tournamentTimeLimitText.node,attrs={'size':4})
                c.connectAttr('output',self._tournamentTimeLimitTitleText.node,'color')
                c.connectAttr('output',self._tournamentTimeLimitText.node,'color')
                bsUtils.animate(c,"input0",{0:1, 150:1},loop=True)
                bsUtils.animate(c,"input1",{0:1, 150:0.5},loop=True)
                bsUtils.animate(c,"input2",{0:0.1, 150:0.0},loop=True)
                c.input3 = 1.0
        ...
        if self._tournamentTimeLimitTime <= 0:
            ...
            n = bs.newNode('text',
                           attrs={'vAttach':'top','hAttach':'center',
                                  'hAlign':'center','color':(1,0.7,0,1),
                                  'position':(0,-200),'scale':1.6,
                                  'text':bs.Lstr(resource='tournamentTimeExpiredText',fallbackResource='timeExpiredText')
                           })
            bs.playSound(bs.getSound('refWhistle'))
            bsUtils.animate(n,"scale",{0:0.0, 100:1.4, 150:1.2})
    ...
    def showZoomMessage(self, message, color=(0.9,0.4,0.0), scale=0.8, duration=2000, trail=False):
        ...
        """
        # reserve a spot on the screen (in case we get multiple of these so they dont overlap)
