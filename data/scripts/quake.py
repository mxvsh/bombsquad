...
class QuakeGame(bs.TeamGameActivity):
    ...
    def getSettings(cls,sessionType):
        settings = [("Kills to Win Per Player",{'minValue':1,'default':15,'increment':1}),
                    ...
                    ("Respawn Times",{'choices':[('At once',0.0),('Shorter',0.25),('Short',0.5),('Normal',1.0),('Long',2.0),('Longer',4.0)],'default':1.0}),
                    ("Speed",{'default':True}),
                    ("Enable jump",{'default':True}),
                    ("Enable pickup",{'default':True}),
                    ("Enable bomb",{'default':False}),
                    ("Obstacles",{'default':True}),
                    ("Obstacles form",{'choices':[('Cube',0.0),('Sphere',1.0),('Random',2.0)],'default':0.0}),
    ...
    def onBegin(self):
        ...
        if self.settings["Obstacles"]:
            ...
            for i in range(count):
                if map == 'Football Stadium':
                    radius = (random.uniform(-10,1),6,random.uniform(-4.5,4.5)) if i > count/2 else (random.uniform(10,1),6,random.uniform(-4.5,4.5))
                else:
                    radius = (random.uniform(-10,1),6,random.uniform(-8,8)) if i > count/2 else (random.uniform(10,1),6,random.uniform(-8,8))
        ...
        if self.settings["Graphics"] == 2:
            ...
            light = bs.newNode('light',
                            attrs={'position':(9,10,0) if map == 'Football Stadium' else (6,7,-2) if not map == 'Rampage' else (6,11,-2) if not map == 'The Pad' else (6,8.5,-2),
                                    'color':(0.4,0.4,0.45),
                                    'radius':1,
                                    'intensity':6,
    ...
    def dropShield(self):
        ...
        p = bs.Powerup(powerupType = 'shield', position = (random.uniform(-10,10),6,random.uniform(-5,5))).autoRetain()
    ...
    def spawnPlayer(self,player):
        ...
        spaz.connectControlsToPlayer(enableJump=self.settings["Enable jump"], enablePunch=True, enablePickUp=self.settings["Enable pickup"], enableBomb=self.settings["Enable bomb"], enableRun=True, enableFly=False)
        ...
        def checkExistsSpaz():
            if spaz.node.exists():
                ...
                print '[QUAKE DEBUG] spazLight deleted'
...
class Boxes(bs.Actor):
    def __init__(self,position=(0,1,0),graphics = 2,randomColor = True,mirror = False,form = 0):
        ...
        self.node = bs.newNode('prop',
                               ...
                               attrs={'position':position,
                                      ...
                                      'materials':[bs.getSharedObject('footingMaterial')] if mirror else [bs.getSharedObject('objectMaterial'),bs.getSharedObject('footingMaterial')]})
                                      
        if graphics == 2:             
            self.lightNode = bs.newNode('light',
                            attrs={'position':(0,0,0),
                                    'color':((0.8,0.2,0.2) if i < count/2 else (0.2,0.2,0.8)) if not randomColor else ((random.random(),random.random(),random.random())),
                                    'radius':0.2,
                                    'intensity':1,
                                    'volumeIntensityScale': 10.0})
