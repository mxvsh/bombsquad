...
class BossBattleGame(bs.CoopGameActivity):
    ...
    def cheat(self):
        ...
        self.spaz.node.handleMessage(bs.PowerupMessage(powerupType = 'health'))
    ...
    def registerBotDie(self,m):
        ...
        if len(self._bots.getLivingBots()) == 0 and not self.stop:
            ...
            self.num += 1
            self.killed = 0
            bs.PopupText(bs.Lstr(value='+${A} ${B}',subs=[('${A}',str(int(bs.getGameTime() - self.time)/1000)),('${B}',bs.Lstr(resource='timeBonusText'))]),
                         color=(1,1,0.5,1),
                         scale=1.0,
                         position=self.spaz.node.position if self.spaz is not None and self.spaz.node.exists() else (0,5,0)).autoRetain()
    ...
    def startNextWave(self,num):
        ...
        for i in range(len(self.waves[self.num])):
            bs.gameTimer(i*100,bs.Call(self._bots.spawnBot,self.waves[self.num][i],pos=self.getRandomPos() if not self.waves[self.num][i] == bs.EricBot else (9, 5 ,-5),spawnTime=1000))
            
        self.showZoomMessage(bs.Lstr(value='${A} ${B}',subs=[('${A}',bs.Lstr(resource='waveText')),('${B}',str(self.num))]),
        ...
        self._waveText = bs.NodeActor(bs.newNode('text',
                                                 attrs={'vAttach':'top',
                                                        ...
                                                        'text':bs.Lstr(value='${A} ${B}',subs=[('${A}',bs.Lstr(resource='waveText')),('${B}',str(self.num))])}))
