...
class _Entry(object):
    ...
    def __init__(self, scoreboard, team, doCover, scale, label, flashLength):
        ...
        if self._doCover:
            if vr: self._backingColor = [0.1+c*0.1 for c in safeTeamColor]
            #if vr: self._backingColor = [0,0,0]
            else: self._backingColor = [0.05+c*0.17 for c in safeTeamColor]
        else: self._backingColor = [0.05+c*0.1 for c in safeTeamColor]

        self._backing = bs.NodeActor(bs.newNode('image',
                                                attrs={'scale':(self._width,self._height),
        ...
        self._bar = bs.NodeActor(bs.newNode('image',
                                            attrs={'opacity':0.7,
                                                   'color':self._barColor,
                                                   'attach':'topLeft',
                                                   'texture':self._barTex}))

        self._barScale = bs.newNode('combine',owner=self._bar.node,
                                    attrs={'size':2,
        ...
        if self._doCover:
            self._cover = bs.NodeActor(bs.newNode('image',
                                                  attrs={'scale':(self._width*1.15,self._height*1.6),
                                                         'opacity':1.0,
        ...
        self._scoreText = bs.NodeActor(bs.newNode('text',
                                                  attrs={'hAttach':'left',
                                                         'vAttach':'top',
                                                         'hAlign':'right',
                                                         'vAlign':'center',
        ...
        if label is not None:
            ...
            if type(teamNameLabel) is bs.Lstr:
                ...
                # hmmm; if the team-name is a non-translatable value lets go ahead and clip it
                # otherwise we leave it as-is so translation can occur..
                if teamNameLabel.isFlatValue():
                    v = teamNameLabel.evaluate()
                    # in python < 3.5 some unicode chars can have length 2, so we need
                    # to convert to raw int vals for safer trimming
        ...
        self._nameText = bs.NodeActor(bs.newNode('text',
                                                 attrs={'hAttach':'left',
                                                        'vAttach':'top',
                                                        'hAlign':'left',
                                                        'vAlign':'center',
    ...
    def flash(self,countdown,extraFlash):
        self._flashTimer = bs.Timer(100,bs.WeakCall(self._doFlash),repeat=True)
    ...
    def _setPosition(self,p):
        ...
        if not self._backing.node.exists(): return
        self._pos = tuple(p)
        self._backing.node.position = (p[0]+self._width/2,p[1]-self._height/2)
        if self._doCover: self._cover.node.position = (p[0]+self._width/2,p[1]-self._height/2)
        self._barPosition.input0 = self._pos[0]+self._barWidth/2
        self._barPosition.input1 = self._pos[1]-self._barHeight/2
        self._scoreText.node.position = (self._pos[0]+self._width-7.0*self._scale, self._pos[1]-self._barHeight+16.0*self._scale)
        self._nameText.node.position = (self._pos[0]+7.0*self._scale, self._pos[1]-self._barHeight+16.0*self._scale)
    ...
    def _setFlashColors(self,f):
        ...
        if f:
            s = 2.0
            _safeSetAttr(self._backing.node,"color",(self._backingColor[0]*s,self._backingColor[1]*s,self._backingColor[2]*s))
            _safeSetAttr(self._bar.node,"color",(self._barColor[0]*s,self._barColor[1]*s,self._barColor[2]*s))
            if self._doCover: _safeSetAttr(self._cover.node,"color",(self._coverColor[0]*s,self._coverColor[1]*s,self._coverColor[2]*s))
        else:
            _safeSetAttr(self._backing.node,"color",self._backingColor)
            _safeSetAttr(self._bar.node,"color",self._barColor)
            if self._doCover: _safeSetAttr(self._cover.node,"color",self._coverColor)
    ...
    def setValue(self,score,maxScore=None,countdown=False,flash=True,showValue=True):

        # if we have no score yet, just set it.. otherwise compare and see if we should flash
        if self._score is None:
            self._score = score
        else:
            if score > self._score or (countdown and score < self._score):
                extraFlash = (maxScore is not None and score >= maxScore and not countdown) or (countdown and score == 0)
                if flash: self.flash(countdown,extraFlash)
            self._score = score

        if maxScore is None:
            self._barWidth = 0.0;
        else:
            if countdown: self._barWidth = max(2.0*self._scale,self._width * (1.0 - (float(score)/maxScore)))
            else: self._barWidth = max(2.0*self._scale,self._width * (min(1.0,float(score)/maxScore)))

        curWidth = self._barScale.input0
        bsUtils.animate(self._barScale,'input0',{0:curWidth,250:self._barWidth})
        self._barScale.input1 = self._barHeight
        curX = self._barPosition.input0
        bsUtils.animate(self._barPosition,'input0',{0:curX,250:self._pos[0]+self._barWidth/2})
        self._barPosition.input1 = self._pos[1]-self._barHeight/2
        if showValue: self._scoreText.node.text = str(score)
        else: self._scoreText.node.text = ''
...
class _EntryProxy(object):
    ...
    def __del__(self):
        ...
        if sb is not None: sb._removeTeam(self._teamID)
...
class ScoreBoard(object):
    ...
    def setTeamValue(self,team,score,maxScore=None,countdown=False,flash=True,showValue=True):
        """
        Update the score-board display for the given team.
        """
        if not team.getID() in self._entries:
            self._addTeam(team)
            # create a proxy in the team which will kill our entry when it dies (for convenience)
            if '_scoreBoardEntry' in team.gameData: raise Exception("existing _EntryProxy found")
            team.gameData['_scoreBoardEntry'] = _EntryProxy(self,team)
        # now set the entry..
        self._entries[team.getID()].setValue(score=score,maxScore=maxScore,countdown=countdown,flash=flash,showValue=showValue)

    def _addTeam(self,team):
        if team.getID() in self._entries: raise Exception('Duplicate team add')
        self._entries[team.getID()] = _Entry(self,team,doCover=self._doCover,scale=self._scale,
                                            label=self._label,flashLength=self._flashLength)
        self._updateTeams()
