...
class QuakeBallFactory(object):
    def __init__(self):
        ...
        self.ballMaterial.addActions(actions=('modifyPartCollision','friction',0))

        self.ballMaterial.addActions(conditions=('theyHaveMaterial', bs.getSharedObject('playerMaterial')),
    ...
    def shot(self,spaz):
        ...
        if time - self.lastShot > 600:
            ...
            QuakeBall(position = spaz.node.position,velocity = (vel[0]*2,vel[1]*2,vel[2]*2),owner = spaz.getPlayer(),sourcePlayer = spaz.getPlayer(),color = spaz.node.color).autoRetain()
...
class QuakeBall(bs.Actor):
    def __init__(self,position = (0,5,0), velocity = (0,2,0), sourcePlayer = None, owner = None,color = (random.random(),random.random(),random.random()),lightRadius = 0):
    ...
    def emit(self):
        bs.emitBGDynamics(position=self.node.position,velocity=self.node.velocity,count=10,scale=0.4,spread=0.01,chunkType='spark');
            ...
            if self.node.exists():
                ...
                explosion = bs.newNode("explosion",
                       attrs={'position':self.node.position,
                              ...
                              'radius':1,
                              'big':False})
                bs.playSound(sound = bs.getSound(random.choice(['impactHard','impactHard2','impactHard3'])),position = self.node.position)
                self.node.delete()
