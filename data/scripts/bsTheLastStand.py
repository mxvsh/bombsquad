...
class TheLastStandGame(bs.CoopGameActivity):
    ...
    def doEnd(self,outcome):
        ...
        self.end(delay=2000,results={'outcome':outcome,'score':self._score.get(),'playerInfo':self.initialPlayerInfo})
    ...
    def _updateScores(self):
        ...
        if self._preset == 'default':
            if score >= 250: self._awardAchievement('Last Stand Master')
            if score >= 500: self._awardAchievement('Last Stand Wizard')
            if score >= 1000: self._awardAchievement('Last Stand God')
